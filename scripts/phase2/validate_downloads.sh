...
if [ "$FAILED_COUNT" -gt 0 ]; then
    echo ""
    echo "To re-download failed genomes:"
    echo "  cp ${FAILED_FILE} accessions_to_download.txt.bak"
    echo "  cp ${FAILED_FILE} accessions_to_download.txt"
    echo "  nohup bash \${SCRIPT_DIR}/download_login.sh > \${SCRIPT_DIR}/download_retry.log 2>&1 &"
    echo "  # Then restore original: cp accessions_to_download.txt.bak accessions_to_download.txt"
fi
